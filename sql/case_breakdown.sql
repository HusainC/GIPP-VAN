...
// NB winning schemes
SET NBRE = $PLACENAME||'_NB_Req_Earnix"';
SET NBWE = $PLACENAME||'_NB_Win_Earnix"';

CREATE OR REPLACE TEMPORARY TABLE IDENTIFIER($NBRE) AS
SELECT
a.NB_Submission
,b.Referral
,b.InsertTimeStamp
...
,b.Commission
,b.CommissionPNCD
,ROW_NUMBER() OVER (PARTITION BY a.NB_Submission, a.Brand ORDER BY a.NetPremium ASC) AS EARNIX_NO
FROM IDENTIFIER($NBRE) a
LEFT JOIN PRD_RAW_DB.QUOTES_PUBLIC.VW_EARNIX_VAN_RES_SCHEME b
  ON a.NB_Submission = b.Quote_Reference
  AND a.Brand = b.Brand
  AND a.Scheme = b.Scheme
WHERE b.Commission != '99999'
AND b.CommissionPNCD != '99999';

SET VCBD = $PLACENAME||'_Van_Case_Breakdown_New"';
...
,CASE
    WHEN e.Cheapest_Scheme = i.Incumbent_Scheme THEN 'Y'
    ELSE 'N'
    END AS "CHEAPEST_SELECTED"
,a.NB_Submission
,n.Brand AS "NB_BRAND"
,n.Scheme AS "NB_SCHEME"
,n.NetPremium AS "NB_NET"
,n.NetPremiumPNCD AS "NB_NETPNCD"
,n.Commission AS "NB_COMMISSION"
,n.CommissionPNCD AS "NB_COMMISSIONPNCD"
,(n.NetPremium + n.Commission) as "NB_GROSS"
,(n.NetPremiumPNCD + n.CommissionPNCD) as "NB_GROSSPNCD"
//,(i.INVITED_COMMISSION / (i.INVITED_NET + i.INVITED_COMMISSION)) AS "INVITED_%"
//,(i.INVITED_COMMISSIONPNCD / (i.INVITED_NETPNCD + i.INVITED_COMMISSIONPNCD)) AS "INVITEDPNCD_%"
